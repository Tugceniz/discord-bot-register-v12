...
client.elevation = message => {
    ...
    if (message.author.id === ayarlar.json) permlvl = 4;
    return permlvl;
};

client.login(ayalar.bottoken)
...
if (tarih > 1296000000) kontrol = ' \`Kayıt Olmaya Uygun.\`'
  ...
  let kanal1 = client.channels.cache.find(x => x.id === kanal);
    ...
    .setTitle("<SUNUCUİSMİ> HOŞGELDİN")//SUNUCU İSMİ 
   ...
   <@&${ayarlar.kayıtcıRolü}> rolündekiler seninle ilgilenecektir.
 
   Hesabın ${kontrol}

    <#Kayıt> odalarında kaydınızı yaptırabilirsiniz.

  Hesabın Oluşturulma Tarihi: \n ⛧ \` ${moment(member.user.createdAt).format("YYYY DD MMMM dddd (hh:mm:ss)")} \`
 `)
    .setTimestamp()
    .setFooter("Developed By Universe")
...
const voiceChannel = (ayarlar.botvoice)
